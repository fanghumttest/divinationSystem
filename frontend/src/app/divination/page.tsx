...
const bgQuery = MAIN_BG_VERSION ? `?v=${encodeURIComponent(MAIN_BG_VERSION)}` : "";
const fallbackBgUrl = R2_BASE_URL
  ? `${R2_BASE_URL}/v1/backgrounds/main-bg.png${bgQuery}`
  : `/api/r2/v1/backgrounds/main-bg.png${bgQuery}`;
...
export default function DivinationPage() {
  ...
  React.useEffect(() => {
    ...
    const t1 = window.setTimeout(() => setNameAnimPhase("hold"), 950);
    const t2 = window.setTimeout(() => setNameAnimPhase("exit"), 3950);
    const t3 = window.setTimeout(() => setStage(2), 4600);
    return () => {
  ...
  React.useEffect(() => {
    const timer = window.setInterval(() => {
      setDisplayProgress((prev) => {
        if (readyToEnter) return Math.min(100, prev + 2.5);
        if (prev < rawPercent) {
    ...
    >
      ...
      <div
        style={{
          ...
          // 手機 stage 4 垂直排版：canvas 上方漸出，讓神像 DOM 卡透過透明區域可見
          // 神像卡底部約在畫面 42% 處，籤桶頂部約 58%，漸層橋接中間空白帶
          maskImage: isMobile && stage === 4
            ? "linear-gradient(to bottom, transparent 0%, transparent 38%, black 50%)"
            : undefined,
          WebkitMaskImage: isMobile && stage === 4
            ? "linear-gradient(to bottom, transparent 0%, transparent 38%, black 50%)"
            : undefined,
        }}
      >
        <NinghuiScene
          ...
          onProgress={(p) => setSceneProgress(p)}
      ...
      {readyToEnter && stage === 1 && (
        ...
        >
          <h1
            style={{
              margin: 0,
              fontSize: "clamp(38px, 13vw, 180px)",
              letterSpacing: "0.2em",
              lineHeight: "normal",
              textAlign: "center",
      ...
      {/* 第六層：詩籤卡片白色玻璃背板（神像同款，純 DOM；置於 Canvas 之下透出） */}
      <div
        style={{
          position: "absolute",
          inset: 0,
          ...
          justifyContent: "center",
          pointerEvents: "none",
          opacity: isSixthLayer ? 1 : 0,
          transition: "opacity 0.5s ease",
        }}
      >
        <div
          style={{
            width: isMobile ? "min(58vw, 260px)" : isNarrowLayout ? "min(42vw, 320px)" : "min(30vw, 360px)",
            height: isMobile ? "min(72vh, 480px)" : isNarrowLayout ? "min(76vh, 520px)" : "min(80vh, 560px)",
            borderRadius: 28,
