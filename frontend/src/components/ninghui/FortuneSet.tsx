...
const FortuneSet: React.FC<FortuneSetProps> = ({
  ...
  // 首幀：計算每個抽屜的實際角度（弧度），寫入 drawerAnglesRef 供外層使用
  const anglesComputedRef = useRef(false);
  useFrame(() => {
    if (anglesComputedRef.current || !drawerAnglesRef || parts.drawers.length === 0) return;
    anglesComputedRef.current = true;
    sceneClone.updateMatrixWorld(true);
    const rootInv = new THREE.Matrix4().copy(sceneClone.matrixWorld).invert();
    const angles = parts.drawers.map((drawer) => {
      const pos = new THREE.Vector3()
        .setFromMatrixPosition(drawer.matrixWorld)
        .applyMatrix4(rootInv);
      return Math.atan2(pos.x, pos.z);
    });
    drawerAnglesRef.current = angles;
  });
