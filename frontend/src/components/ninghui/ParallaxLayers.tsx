...
const MAIN_BG_TEXTURE = assetUrl(
  MAIN_BG_VERSION
...
const ParallaxLayers = ({
  ...
  // 手機垂直排版（stage 4）：籤筒在下半部，熱點改放籤筒底部（y 為負）
  // fortuneIdleBaseY > -20 是辨識「mobile stage 4 垂直模式」的條件（其他 stage 都 <= -38）
  // fortuneIdleBaseY > -35：捕捉手機 stage 4 垂直排版（y=-30），排除其他 stage（y<=-38）
  const mobileBottomLayout = isMobile && fortuneIdleBaseY > -55;
  // 手機 stage 4：擲筊流程中隱藏 3D 籤桶，筊杯 / 結束求籤熱點置中
  // （drawingPhase 必須為 idle 才套用，picked/closeup 另有其他邏輯）
  const mobileJiaoHidden =
    mobileBottomLayout && jiaoPhase !== "idle" && drawingPhase === "idle";
  // 熱點 y（local space）：
  //   - 手機擲筊隱藏籤桶：貼近畫面底部（world y ≈ -25.6 → 畫面 ~85%）
  //   - 手機垂直排版：放籤桶底部
  //   - 其他：放頂部
  //   注意：hotspot 位於 z=22（group z=8 + local z=7.78），比 z=0 更靠近相機，
  //   透視半高 ≈ 36.44，若 world y 太負（< -36）會被畫面底邊裁掉
  const tubeLabelY = mobileJiaoHidden ? 8 : mobileBottomLayout ? 12 : 25;
  // 垂直模式下補正 TUBE_LABEL_OFFSET_X(-4) 的偏移，使熱點置中
  const tubeLabelXAdj = mobileBottomLayout ? 4 : 0;
  ...
  }, [drawingPhase, pickedStickId, onPickedStickChange]);
  ...
  React.useEffect(() => {
    ...
    if (fortuneGroupRef.current) {
      fortuneGroupRef.current.position.set(22, -40, 28);
      fortuneGroupRef.current.scale.setScalar(1.4 * 1.5);
      fortuneGroupRef.current.rotation.set(0, 0, 0);
    }
    fifthLayerAnimRef.current = { active: false, startX: 22, startY: -40, startZ: 28, startScale: 1.4 * 1.5, progress: 1 };
    setJiaoPhase("idle");
    setJiaoResult(null);
  ...
  const handleAskAgain = React.useCallback(() => {
    ...
    onJiaoActiveChange?.(true);
  }, [onPickedStickChange, onJiaoActiveChange]);
  ...
  const handleStartSecondJiao = React.useCallback(() => {
    ...
    jiaoVyRef.current = 0;
    if (jiaoPreviewRef.current) {
      // 第二輪：籤筒退到右下角，「第N首」籤詩標籤在畫面右側（3D x≈+10）。
      // 筊杯往左偏，避免飄移時壓住標籤。
      jiaoPreviewRef.current.position.set(-8, JIAO_BASE_Y, JIAO_BASE_Z);
    }
  ...
  /** 第三層展示結束後 1 秒自動收場（設 confirmStickId）用的計時器 */
  ...
  const handleDownloadCard = React.useCallback(async () => {
    ...
    const src =
      poemType === "rensheng"
        ? assetUrl(`/v1/cards/rensheng/${confirmPoemId}.webp`)
  ...
  React.useEffect(() => {
    ...
    sixthLayerTimerRef.current = window.setTimeout(() => {
      ...
      const cardP0 = drawerMouthWorldPosRef.current
        ...
        : fallbackP0;
      // 控制點：往上拱
      const cardP1 = new THREE.Vector3(0, isMobile ? 8 : 10, isMobile ? 26 : 30);
      // 終點：畫面中央偏上（手機靠近鏡頭、縮小以符合視錐）
      const cardP2 = new THREE.Vector3(0, isMobile ? 4 : 2, isMobile ? 30 : 34);
  ...
  }, [openDrawerIndex, isFifthLayer, confirmPoemId, poemType]);
  ...
  useFrame((state, delta) => {
    ...
    //    closeup 時不更新（由 6.5 控制）；picked 且籤尚未抽完時 x/y 不動，抽完後才追基礎位置
    const stickStillDrawing = drawingPhase === "picked" && drawnStickProgressRef.current < 1;
    if (fortuneGroupRef.current && drawingPhase !== "closeup" && !stickStillDrawing) {
      ...
      const targetX = jiaoPhase === "idle"
        ? baseX + tx * 2.2 * mMul
        : (inJiao && !skipJiaoShift) ? baseX + (mobileBottomLayout ? 8 : 24) : baseX;
      const targetY = jiaoPhase === "idle"
        ? baseY + ty * 0.9 * mMul
        : (inJiao && !skipJiaoShift) ? baseY - (mobileBottomLayout ? 6 : 12) : baseY;
    ...
    if (jiaoPreviewRef.current) {
      ...
      // 手機擲筊隱藏籤桶：筊杯置中並下移 10（x=0, y=-10）；否則用原本的 JIAO_BASE
      const baseX = isSecondRoundAiming ? -2 : (mobileJiaoHidden ? 0 : JIAO_BASE_X);
      const baseY = mobileJiaoHidden ? -10 : JIAO_BASE_Y;
      ...
      if (jiaoPhase === "aiming") {
        ...
        // 累積上下晃動量（只看 aimY 的變化）
        const prevTy = jiaoLastTyRef.current;
        const dy = aimY - prevTy;
        jiaoLastTyRef.current = aimY;

        // jiaoPrevPointerDownRef 保留但不用於追蹤，僅用於未來擴充
        jiaoPrevPointerDownRef.current = pointerDownRef?.current === true;

        // 加上阻尼，避免一次晃動後值一直累積
        jiaoShakeRef.current = jiaoShakeRef.current * 0.9 + Math.abs(dy);

        // 超過門檻就觸發「擲出」動作；
        // 1) 累積搖晃達到門檻；2) 單幀上下速度夠大（快速一甩）任一條件皆可擲出
        const threshold = 0.25;
        const instantFlick = Math.abs(dy) > 0.18;
        if (jiaoShakeRef.current > threshold || instantFlick) {
          jiaoShakeRef.current = 0;
          jiaoVyRef.current = 22; // 起始向上速度
          setJiaoPhase("flying");
        }
        ...
        if (!isSecondRound) {
          ...
          // 第二輪（確認詩籤後）：筊杯同樣浮到中央偏左（避免壓住右側「第N首」標籤）
          jiaoPreviewRef.current.position.x = lerp(
            jiaoPreviewRef.current.position.x,
            -1,
            0.08
    ...
    if (fortuneGroupRef.current) {
      ...
      if (drawingPhase === "closeup") {
        ...
        if (isSixthLayer && !sixthLayerAnimRef.current?.active) {
          ...
          // 第五層且進場動畫已結束：固定維持籤筒位置（右側）與放大 0.2 倍
          fortuneGroupRef.current.position.set(22, -40, 28);
          fortuneGroupRef.current.scale.setScalar(1.4 * 1.5);
      ...
      if (isFifthLayer && fifthLayerAnimRef.current?.active && fortuneGroupRef.current) {
        ...
        const eased = t * t * (3 - 2 * t);
        const targetX = 22; // 第五層籤筒維持在右側
        const targetY = -40;
        const targetZ = 28;
        const targetScale = 1.4 * 1.5; // 第五層籤筒放大 0.5 倍（與 setScalar(1.4 * 1.5) 一致）
        const fg = fortuneGroupRef.current;
    ...
    if (isSixthLayer && sixthLayerAnimRef.current?.active && fortuneGroupRef.current) {
      ...
      if (poemCardGroupRef.current) {
        const u = eased; // 使用 eased 讓曲線也有 smoothstep 效果
        const u1 = 1 - u;
    ...
    // 第六層動畫結束後：維持詩籤 scale（含使用者放大縮小）並確保可見
    ...
    if (drawingPhase === "picked" && pickedStickId !== null && drawnStickGroupRef.current) {
      ...
      const end = drawnStickEndRef.current;
      ...
      if (t >= 1) {
        if (confirmStickId !== null) {
          ...
          g.scale.setScalar(DISPLAY_SCALE_FOURTH);
        } else {
          g.position.copy(end);
          g.scale.setScalar(DISPLAY_SCALE);
        }
        if (!labelNotifiedRef.current) {
          // 1) 通知主組件顯示右側「第 N 首」
          onPickedStickChange?.(pickedStickId);
          labelNotifiedRef.current = true;
          // 2) 啟動第三層 → 第四層的 1 秒自動收場流程
        ...
        const s = THREE.MathUtils.lerp(TUBE_SCALE, DISPLAY_SCALE, localT);
    ...
    if (
      ...
      if (retreat?.active) {
        ...
        // 目標值與 closeup 動畫一致（line 1184–1187）
        const closeupTargetX = fortuneIdleBaseX - 2;
        ...
        fg.position.x = THREE.MathUtils.lerp(fg.position.x, closeupTargetX, 0.06);
        fg.position.y = THREE.MathUtils.lerp(fg.position.y, closeupTargetY, 0.06);
        fg.position.z = THREE.MathUtils.lerp(fg.position.z, closeupTargetZ, 0.06);
        fg.scale.setScalar(THREE.MathUtils.lerp(fg.scale.x, closeupTargetScale, 0.06));
      } else {
    ...
    <>
      ...
      >
        ...
        {confirmStickId !== null && flowMode === "fortune" && !isRewindingStick && !isFifthLayer ? (
          <>
            <Hotspot
              label="結束求籤"
              position={tubeTopPos(isSixthLayer ? (isMobile ? -4 : -7) : tubeLabelXAdj, isSixthLayer ? 27 : tubeLabelY, 14)}
              size={[10, 12]}
              onClick={() => {
                setConfirmStickId(null);
                setConfirmPoemId(null);
            ...
            {isSixthLayer && (
              <Hotspot
                label="再問一題"
                position={tubeTopPos(isMobile ? 9 : 13, 27, 14)}
                size={[10, 12]}
          ...
          <>
            ...
            {flowMode === "throw_only" && jiaoPhase !== "idle" && (
              <Hotspot
                label="結束擲筊"
                position={tubeTopPos(tubeLabelXAdj, tubeLabelY, 14)}
                size={[10, 12]}
                onClick={() => {
                  setFlowMode("idle");
                  setJiaoPhase("idle");
              ...
              <>
                {!shengBeiInFortuneFlow || isWaitingQuestionType ? (
                  ...
                  <>
                    <Hotspot
                      label="問修道"
                      position={tubeTopPos((mobileBottomLayout ? -6 : -10) + tubeLabelXAdj, tubeLabelY, 14)}
                      size={[10, 12]}
                    ...
                    <Hotspot
                      label="問人事"
                      position={tubeTopPos((mobileBottomLayout ? 6 : 2) + tubeLabelXAdj, tubeLabelY, 14)}
                      size={[10, 12]}
            ...
            {flowMode === "idle" && jiaoPhase === "idle" && isTubeMenuVisible && (
              <>
                <Hotspot
                  label="求籤"
                  position={tubeTopPos(-6 + tubeLabelXAdj, tubeLabelY, 14)}
                  size={[24, 28]}
      ...
      {drawingPhase === "picked" && pickedStickId !== null && (
        <group ref={drawnStickGroupRef} layers={1} position={[0, 0, 0]} renderOrder={10}>
          ...
          {/* 「第 N 首」只保留主組件固定 overlay，不在此重複顯示，避免動畫時出現多組殘影 */}
        </group>
      )}
      ...
      {drawingPhase === "picked" &&
        ...
        !secondJiaoStarted &&
        !isRewindingStick && (
          <group position={[0, -14, 12]} scale={6.6}>
            <group scale={12.5}>
      ...
      {isSixthLayer && confirmPoemId != null && poemType && (
        <>
          ...
          <Hotspot
            label="儲存圖片"
            position={[0, isMobile ? -3 : -5, isMobile ? 31 : 35]}
            size={[10, 12]}
          ...
          {/* 放大縮小按鈕 */}
          <Html
            position={[isMobile ? 6 : 8, isMobile ? -3 : -5, isMobile ? 31 : 35]}
            center
            zIndexRange={[4000, 0]}
            style={{ pointerEvents: "auto" }}
          >
